var DDeliveryIframe = (function () {
    ...
    return {
        ...
        init: function (_componentUrl, _staticUrl) {
            ...
            if (window.params && window.params.city_id > 0) {
	            data.city_id = window.params.city_id;
            }
            this.ajaxPage(data);

            $('#ddelivery_loader .load_error a').click(repeatLastQuery);
            if (window.params && window.params.hideCloseButton) {
                var style = document.createElement('STYLE');
                style.innerHTML = ".map-popup__head__close {display: none !important;}";
        ...
        ajaxPage: function (data) {
            ...
            if (data != null && data.action != null && data.action.length > 0) {
	            queryUrl += '/' + data.action;
            }
            data.shippingId = window.shippingId;

        	$.post(queryUrl, data, function (dataHtml) {
        ...
        ajaxData: function (data, callBack) {
            ...
            if (this.orderId)
                data.order_id = this.orderId;
            $.post(queryUrl, data, function (result) {
