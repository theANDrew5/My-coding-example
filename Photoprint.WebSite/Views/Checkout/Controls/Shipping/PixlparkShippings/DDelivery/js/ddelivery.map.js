    ...
    return {
        ...
        filterPoints: function () {
            ...
            var isDisplayPoint = function (point) {
                ...
                if (!((filter.card && point.IsCard) || (filter.cash && point.IsCash))) {
                    return false;
                }
                ...
                if (filter.has_fitting_room && !point.HasFittingRoom) {
                    return false;
                }

                if (filter.hideCompany.indexOf(point.CompanyId) != -1) {
                    return false;
                }
                return true;
            };
        ...
        event: function () {
            ...
            $('.map-popup__info__btn a').click(function(){
            	if (!selectedPoint || !selectedCompany)
                    return;
                DDeliveryIframe.postMessage('mapPointChange', {
                	point: selectedPoint,
        ...
        placeEvent: function () {
            $('.map-popup__main__right .places a').click(function () {
                if (current_points.length > 0) {
                    ...
                    if (current_point.CompanyId != parseInt($(this).data('id'))) {
        ...
        renderData: function (data) {
            ...
            if(typeof(data.headerData) != 'undefined') {
                for(var key in data.headerData ) {
                    ...
                    $('.delivery-type__drop_'+key+' .date span').html('дн.');
                }
            }
        ...
        renderInfo: function (point, points) {
            ...
            if (points.length > 1) {
                ...
                for (var i = 0; i < points.length; i++) {
                    $('.map-popup__main__right .places a[data-id=' + points[i].CompanyId + ']').addClass('hasinfo');
                }
                $('.map-popup__main__right .places a[data-id=' + point.CompanyId + ']').addClass('active');
            } else {
                $('.map-popup__info__title .more').hide();
                $('.map-popup__main__right .places a[data-id=' + point.CompanyId + ']').addClass('active').addClass('hasinfo');
            }

            if (!point.Name) {
            	point.Name = point.Company + ' #' + point.Id;
            }
            ...
            DDeliveryIframe.ajaxData(
                {action: 'mapGetPoint', id: point.Id, 'custom': point.IsCustom ? 1 : '', city_id: $('input[name=ddelivery_city]').val()},
                function (data) {
                	if (typeof (data.length) == 'undefined') { // object
                		...
                		$('.map-popup__info__table .rub').html(data.company.TotalPriceCorrected);
                        var day = $('.map-popup__info__table .day').show();
                        $('strong', day).html(data.company.DeliveryTimeMinCorrected);
                        $('span', day).html('дн.');
                        if(data.point.IndoorPlace)
                        	$('.address', more).html(point.Address + ', ' + data.point.IndoorPlace);


                        $('.schedule', more).html(data.point.Schedule.replace(/\n/g, "<br>"));

                        var description = (data.point.DescriptionIn + '<br/>' + data.point.DescriptionOut).replace(/\n/g, '<br/>');
